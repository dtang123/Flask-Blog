...
class BlogPost(db.Model):
    ...
    comments = relationship("Comments", back_populates="comment_article")
...
class User(db.Model, UserMixin):
    ...
    #Parent to Comments
    comment_posts = relationship("Comments", back_populates="c_post_author")
...
class Comments(db.Model):
    ...
    #Child to Users
    author = db.Column(db.Integer, db.ForeignKey("users.id"))
    c_post_author = relationship("User", back_populates="comment_posts")
    #Child to BlogPost
    article = db.Column(db.Integer, db.ForeignKey("blog_posts.id"))
    comment_article = relationship("BlogPost", back_populates="comments")
...
def register():
    ...
    if registerForm.validate_on_submit():
        encrypted_pw = generate_password_hash(registerForm.password.data, method="pbkdf2:sha256", salt_length=8)
        newUser = User(
...
def show_post(post_id):
    ...
    comments_list = Comments.query.filter_by(article=post_id).all()
    ...
    return render_template("post.html", post=requested_post, form=comm_form, gravatar=gravatar, comments=comments_list, user=current_user)
